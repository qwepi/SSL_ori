...
if aug==1:
    predict, predict1 = forward_new(x, flip=True)   
elif fortest == 0:
    predict,predict1 = forward_new(x)
else:
    predict,predict1 = forward_new(x, is_training = False)                                                       #do forward
loss_w   = tf.nn.softmax_cross_entropy_with_logits(labels=y_gt, logits=predict) 
loss   = tf.reduce_mean(loss_w*W)                                                             #calc batch loss
...
lr     = 0.0003
dr     = 0.5 #learning rate decay rate
...
with tf.Session(config=config) as sess:
    ...
    for t in xrange(4):
        ...
        run_test()
        if t ==3:
            path = "%smodel-t%d-p%g-s%d-step%d-SSL-dac-newforward-881616.ckpt" % (save_path, t, train_ratio, seed, step)
            print("save to path:", path)
            saver.save(sess, path)
      
        if train_ratio ==1:
            path = "%smodel-t%d-p%g-s%d-step%d-SSL-dac-test.ckpt" % (save_path, t, train_ratio, seed, step)
            print("save to path:", path)
            saver.save(sess, path)
        ...
        print("*******get unlabeled data for next training, ten round*******")
        #define r and choose data for next iteration
        #training model(need X, predict Y and weight)
        un_wholeXYwl = list(zip(unX,un_p_label,wi,un_loss1))
        #for i in range(unlabel_wholeXYwl):
        un_wholeXYwl.sort(key = lambda x:x[3])
        num_k = int(len(un_wholeXYwl)/10+1)
        un_batches = [un_wholeXYwl[k:k+num_k] for k in range(0, len(un_wholeXYwl), num_k)]
        un_for_r = []
        un_for_r_acc = []
        bar = Bar('training model with unlabeled data to define r', max=10)
        for i in range(10):
            ...
            for step in xrange(maxitr_un):
                ...
                batch_wi   = batch[2]
                batch_label_all_without_bias = processlabel(batch_label)
                feed_dict = {x_data: batch_data, y_gt: batch_label_all_without_bias, W:batch_wi, lr_holder:lr, fortest:0}
                training_loss, training_acc, _ = sess.run([loss, accu, opt_cnn], feed_dict=feed_dict)
                learning_rate = lr
                #print("###################### checkpoint 10")
        ...
        print("%d unlabeled data are used" % (len(unlabel_fortrain)))
