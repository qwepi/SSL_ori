...
def svm_labelprocess(Y):
    ...
    for i in Y:
        ...
        if i ==0:
            Y_svm.append(-1)
    return Y_svm
def svm_reverse_labelprocess(Y):
    Y_r = []
    for i in Y:
        if i ==1:
            Y_r.append(1)
...
def batchfor_test_svm(testX,testY,num,batch=1000):
    testlen=len(testX)
    if batch>testlen:
        print('ERROR:Batch size exceeds data size')
        print('Abort.')
...
#first round weight modification
def weight_modify(un_p_label,wi):
    for i in range(len(un_p_label)):
        if un_p_label[i]==1:
            wi[i] = 1- wi[i]
    return wi

#hotspot flipping
def traindata_flip(trainX,trainY):
    newX_h = []
    newX_n = []
    #pdb.set_trace()
    for i in range(len(trainY)):
        if trainY[i] == 1:
            newX_h.append(trainX[i])
        else:
            newX_n.append(trainX[i])
    #pdb.set_trace()
    #newX_h = np.reshape(np.array(newX_h),[-1,12,12,32])
    #newX_n = np.reshape(np.array(newX_n),[-1,12,12,32])
    newX_h1 =[]
    for i in range(len(newX_h)):
        testX = np.fliplr(newX_h[i])
        testX1 = np.flipud(newX_h[i])
        testX2 = np.flipud(np.fliplr(newX_h[i]))
        newX_h1.append(np.fliplr(newX_h[i]))
        newX_h1.append(np.flipud(newX_h[i]))
        newX_h1.append(np.flipud(np.fliplr(newX_h[i])))
    #pdb.set_trace()
    #newX_h1 = np.array(newX_h1)
    newX_h = list(newX_h)
    newX_h.extend(newX_h1)
    newX = []
    newX.extend(newX_h)
    newX.extend(list(newX_n))
    newX = np.array(newX)
    newY = np.concatenate((np.ones(len(newX_h)),np.zeros(len(newX_n))))
    return newX, newY

#non hotspot flipping
def traindata_flip_nhs(trainX,trainY):
    newX_h = []
    newX_n = []
    #pdb.set_trace()
    for i in range(len(trainY)):
    ...
    #newX_n = np.reshape(np.array(newX_n),[-1,12,12,32])
    newX_n1 =[]
    for i in range(len(newX_n)):
        testX = np.fliplr(newX_n[i])
        testX1 = np.flipud(newX_n[i])
        testX2 = np.flipud(np.fliplr(newX_n[i]))
        newX_n1.append(np.fliplr(newX_n[i]))
        newX_n1.append(np.flipud(newX_n[i]))
        newX_n1.append(np.flipud(np.fliplr(newX_n[i])))
    #pdb.set_trace()
    #newX_h1 = np.array(newX_h1)
    newX_n = list(newX_n)
    newX_n.extend(newX_n1)
    newX = []
    newX.extend(list(newX_h))
    newX.extend(newX_n)
    newX = np.array(newX)
    newY = np.concatenate((np.ones(len(newX_h)),np.zeros(len(newX_n))))
    return newX, newY
...
# get batch for training, with bias and weight
# new, choose n and h from the whole part, not seperately, similar to sgd_batch
def get_batch_withweight_bias_sgd(trainX,trainY,weight,batchsize=32):
    #train_nh_X, train_h_X, weight_nh,weight_h = get_batch_nh(trainX,trainY,weight)
    #batchsize = min(len(train_nh_X), len(train_h_X), batchsize)
    #pdb.set_trace()
    labexn = np.where(trainY==0)[0]
    labexh = np.where(trainY==1)[0]
    ...
    if h_length < num:
        if not h_length ==0:
            idxn = labexn[(np.random.rand(batchsize-h_length)*n_length).astype(int)]
            ft_batch = np.concatenate((trainX[idxn],trainX[labexh]))
            label = np.concatenate((np.zeros(batchsize-h_length), np.ones(h_length)))
            weight_whole = np.concatenate((weight[idxn],weight[labexh]))
            ft_batch_nhs = trainX[idxn]
            label_nhs = np.zeros(batchsize-h_length)
            weight_nhs = weight[idxn]
        else:
            idxn = labexn[(np.random.rand(batchsize)*n_length).astype(int)]
            ft_batch = trainX[idxn]
            label = np.zeros(batchsize)
            weight_whole = weight[idxn]
            ft_batch_nhs = ft_batch
            label_nhs = label
            weight_nhs = weight_whole
        ...
        ft_batch = np.concatenate((trainX[idxn], trainX[idxh]))
        label = np.concatenate((np.zeros(num), np.ones(num)))
        weight_whole = np.concatenate((weight[idxn],weight[idxh]))
        ft_batch_nhs = trainX[idxn]
        label_nhs = np.zeros(num)
        weight_nhs = weight[idxn]
    #print(ft_batch.shape, ft_batch_nhs.shape)    
...
# get batch for training(cancle the nhs part for delta)
def get_batch_withweight_sgd(trainX,trainY,weight,batchsize=32):
    #pdb.set_trace()
    #train_nh_X, train_h_X, weight_nh,weight_h = get_batch_nh(trainX,trainY,weight)
    #batchsize = min(len(train_nh_X), len(train_h_X), batchsize)
    labexn = np.where(trainY==0)[0]
    ...
    if h_length < num:
        ...
        if not n_length ==0:
            idxh = labexh[(np.random.rand(batchsize-n_length)*h_length).astype(int)]
            ft_batch = np.concatenate((trainX[idxh],trainX[labexn]))
            label = np.concatenate((np.ones(batchsize-n_length), np.zeros(n_length)))
            weight_whole = np.concatenate((weight[idxh],weight[labexn]))
        else:
            idxh = labexh[(np.random.rand(batchsize)*h_length).astype(int)]
            ft_batch = trainX[idxh]
            label = np.ones(batchsize)
            weight_whole = weight[idxh]
    else:
        #num = batchsize/2
        idxn = labexn[(np.random.rand(num)*n_length).astype(int)]
        idxh = labexh[(np.random.rand(num)*h_length).astype(int)]
...
def get_batch_withweight_bias_nextbatch_beta(trainX,trainY,weight,batchsize=32):
...
def get_batch_noweight(trainX,trainY,batchsize=32,ifsvm = 0):
    #pdb.set_trace()
    if ifsvm == 0:
        train_nh_X, train_h_X = get_batch_nh_noweight(trainX,trainY)
    else:
        train_nh_X, train_h_X = get_batch_nh_noweight_svm(trainX,trainY)
    #batchsize = min(len(train_nh_X), len(train_h_X), batchsize)
    n_length = len(train_nh_X)
    h_length = len(train_h_X)
    if h_length < 16:
...
#get Dkl
def get_batch_nh_noweight_svm(trainX,trainY):
    trainwhole = list(zip(trainX,trainY))
    train_nh_X = []
    train_h_X = []
    for i in trainwhole:
...
def get_data_un_r(un_for_r):
    ...
    for i in un_for_r:
        ...
        W.append(z)
    return X,Y,W
def get_data_un_r_SVM(un_for_r):
    X=[]
    Y=[]
    for i in un_for_r:
        x,y,z = i
...
def loss_to_bias(loss,  alpha, threshold=0.3):
    #print("t=%d,threshold=%f"%(t,threshold))
    if loss >= threshold:
        bias = 0
    else:
...
def forward_stopgradient(input, is_training=True, reuse=False, scope='model', flip=False):
    #if flip == True:
        #input = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input)
        #input = tf.map_fn(lambda img: tf.image.random_flip_up_down(img), input)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1, padding='SAME',
            ...
            #crosstask part
            #print gradient
            #grad1 = tf.gradients(net,[input])
            #print("grad1=",grad1)
            #net1 = tf.stop_gradient(net)
            #net1 = slim.conv2d(net1, 32, [3, 3], scope='conv2_1_1')
            #net1 = slim.conv2d(net1, 32, [3, 3], scope='conv2_2_1')
            #net1 = slim.max_pool2d(net1, [2, 2], stride=2, padding='SAME', scope='pool2_1')
            #net1 = slim.flatten(net1)
            #grad2 = tf.gradients(net,[input])
            #print("grad2=",grad2)
            net = slim.conv2d(net, 32, [3, 3], scope='conv2_1')
            net = slim.conv2d(net, 32, [3, 3], scope='conv2_2')
            net = slim.max_pool2d(net, [2, 2], stride=2, padding='SAME', scope='pool2')
...
def forward_second(input, is_training=True, reuse=False, scope='model', flip=False):
    #if flip == True:
        #input = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input)
        #input = tf.map_fn(lambda img: tf.image.random_flip_up_down(img), input)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1, padding='SAME',
                            weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                            biases_initializer=tf.constant_initializer(0.0)):
            net = slim.conv2d(inputs=input, num_outputs=16, kernel_size=[3, 3], scope='conv1_1_s')
            net = slim.conv2d(net, 16, [3, 3], scope='conv1_2_s')
            net = slim.max_pool2d(net, [2, 2], stride=2, padding='SAME', scope='pool1_s')

            net = slim.conv2d(net, 32, [3, 3], scope='conv2_1_s')
            net = slim.conv2d(net, 32, [3, 3], scope='conv2_2_s')
            net = slim.max_pool2d(net, [2, 2], stride=2, padding='SAME', scope='pool2_s')
            net = slim.flatten(net)
            w_init = tf.contrib.layers.xavier_initializer(uniform=False)
            net = slim.fully_connected(net, 250, activation_fn=tf.nn.relu, scope='fc1_s')
            net = slim.dropout(net, 0.5, is_training=is_training, scope='dropout_s')
            predict = slim.fully_connected(net, 2, activation_fn=None, scope='fc2_s')
            #crosstask part
    return predict

def forward_cluster(input, is_training=True, reuse=False, scope='model', flip=False):
    #if flip == True:
        #input = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input)
        #input = tf.map_fn(lambda img: tf.image.random_flip_up_down(img), input)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1, padding='SAME',
                            weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                            biases_initializer=tf.constant_initializer(0.0)):
            net = slim.conv2d(inputs=input, num_outputs=16, kernel_size=[3, 3], scope='conv1_1')
            net = slim.conv2d(net, 16, [3, 3], scope='conv1_2')
            net = slim.max_pool2d(net, [2, 2], stride=2, padding='SAME', scope='pool1')
            #crosstask part
            net1 = slim.conv2d(net, 16, [3, 3], scope='conv2_1_1')
            #net1 = slim.conv2d(net1, 16, [3, 3], scope='conv2_2_1')
            net1 = slim.max_pool2d(net1, [2, 2], stride=2, padding='SAME', scope='pool2_1')
            net1 = slim.flatten(net1)
            net = slim.conv2d(net, 32, [3, 3], scope='conv2_1')
            net = slim.conv2d(net, 32, [3, 3], scope='conv2_2')
...
def forward_splitfirst(input, is_training=True, reuse=False, scope='model', flip=False):
    #if flip == True:
        #input = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input)
        #input = tf.map_fn(lambda img: tf.image.random_flip_up_down(img), input)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1, padding='SAME',
            ...
            net = slim.conv2d(inputs=input, num_outputs=16, kernel_size=[3, 3], scope='conv1_1')
             #crosstask part
            net1 = slim.conv2d(net, 16, [3, 3], scope='conv1_2_1')
            net1 = slim.max_pool2d(net1, [2, 2], stride=2, padding='SAME', scope='pool1_1')
            net1 = slim.conv2d(net1, 32, [3, 3], scope='conv2_1_1')
            net1 = slim.conv2d(net1, 32, [3, 3], scope='conv2_2_1')
            net1 = slim.max_pool2d(net1, [2, 2], stride=2, padding='SAME', scope='pool2_1')
            net1 = slim.flatten(net1)
...
def forward_dropout(input, is_training=True, reuse=False, scope='model', flip=False):
    #if flip == True:
        #input = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input)
        #input = tf.map_fn(lambda img: tf.image.random_flip_up_down(img), input)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1, padding='SAME',
            ...
            net1 = slim.conv2d(net1, 32, [3, 3], scope='conv2_2_1')
            net1 = slim.max_pool2d(net1, [2, 2], stride=2, padding='SAME', scope='pool2_1')
            net1 = slim.dropout(net1, 0.75, is_training =is_training, scope = 'dropout2')
            net1 = slim.flatten(net1)
            net = slim.conv2d(net, 32, [3, 3], scope='conv2_1')
            net = slim.conv2d(net, 32, [3, 3], scope='conv2_2')
            net = slim.max_pool2d(net, [2, 2], stride=2, padding='SAME', scope='pool2')
            net = slim.dropout(net, 0.75, is_training =is_training, scope = 'dropout3')
            net = slim.flatten(net)
            w_init = tf.contrib.layers.xavier_initializer(uniform=False)
            net = slim.fully_connected(net, 250, activation_fn=tf.nn.relu, scope='fc1')
            net = slim.dropout(net, 0.5, is_training=is_training, scope='dropout')
