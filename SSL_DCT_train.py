...
if aug==1:
    predict, predict1 = forward(x, flip=True)   
elif fortest == 0:
    predict,predict1 = forward(x)
else:
    predict,predict1 = forward(x, is_training = False)                                                       #do forward
...
with tf.Session(config=config) as sess:
    ...
    for t in xrange(4):
        ...
        #run_test(step)
        if t ==3:
            path = "%smodel-t%d-p%g-s%d-step%d.ckpt" % (save_path, t, train_ratio, seed, step)
            print("save to path:", path)
            saver.save(sess, path)
