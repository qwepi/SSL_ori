...
'''
#Ying
#label propagetion (with E distance)
#len_l = len(trainX)
#len_u = len(unX)
#len_matrix = len(trainX)+len(unX)
#w_lp = np.zeros(len_matrix,len_matrix)


pdb.set_trace()
#get E distance matrix
wholeX_original = np.concatenate((trainX,unX))
#cut 12*12*32 to 12*12*10 
wholeX = wholeX_original[:,:,:,[0,10]]
len_matrix = len(wholeX)
w_lp = np.zeros([len_matrix,len_matrix])
distance = np.zeros([len_matrix,len_matrix])
for i in range(len_matrix):
    print("i=",i)
    for j in range(len_matrix):
        distance_minus = wholeX[i]-wholeX[j]
        distance_square = distance_minus**2
        distance_sum = np.sum(distance_square)
        distance[i][j] = distance_sum
pdb.set_trace()

'''
# seems not working due to the unmatched number 

#fix memory problem
#cut whole dataset into ten subset
num_Xsub = 10
wholeX_batches = [wholeX[k:k+num_Xsub]] for k in range(0, len(wholeX),num_Xsub)]
for i in range(num_Xsub):
    len_data = len(wholeX_batches[i])
    data_matrix = (np.tile(wholeX_batches[i], [len_data,1])).reshape(len_data,len_data,12,12,2)
    data_matrix_transpose(data_matrix,[1,0,2,3,4])
    distance_minus = data_matrix_transpose - data_matrix
    distance_square = distance_minus**2
    distance = [np.sum(each_distance) for each_distance in distance_square]
    for j in range(num_Xsub):
        len_data_2 = len(wholeX_batches[j])
        data_matrix = (np.tile(wholeX_batches[i], [len_data,1])).reshape(len_data,len_data,12,12,2)
        data_matrix_transpose(data_matrix,[1,0,2,3,4])
        distance_minus = data_matrix_transpose - data_matrix
        distance_square = distance_minus**2
...
#    
#
#label_matrix = (np.tile(trainX,[len(trainX),1])).reshape(len(trainX),len(trainX),12,12,32)
#un_matrix = (np.tile(unX,[len(unX),1])).reshape(len(unX),len(unX),12,12,2)
#
#data_matrix = (np.tile(wholeX, [len_matrix,1])).reshape(len_matrix,len_matrix,12,12,32)
#data_matrix_transpose(data_matrix,[1,0,2,3,4])
#distance_minus = data_matrix_transpose - data_matrix
#distance_square = distance_minus**2
#distance = [sum(each_distance) for each_distance in distance_square]


#get d0 and o_constant
#d0, the shortest d between two instances with different labels
pc_labeled_whole = pairwise_constraint(trainX,trainY)
distance_labeled_row = distance[[0,len(trainX)]]
distance_labeled = distance_labeled_row[:,[0,len(trainX)]]#d_ll
whole_one = np.ones_like(pc_labeled_whole)
indicator_difflabel = whole_one - pc_labeled_whole
distance_labeled_diff = distance_labeled * indicator_difflabel #get d of different labels
d_index = distance_labeled_diff.nonzero()
d0 = min(distance_labeled_diff[d_index])
o_constant = d0/3
pdb.set_trace()
#get weight_LP matrix
weight_LP_matrix = exp(np.negative(distance/(o_constant^2)))

#get probabilistic transition matrix T
T_matrix = weight_LP_matrix/np.sum(weight_LP_matrix,1)
T_norm = T_matrix/np.sum(T_matrix,1)

T_uu_row = T_norm[len(trainX),-1]
T_uu = T_uu_row[:,[len(trainX),-1]]

T_ul_row = T_norm[0:len(trainX)]
T_ul = T_ul_row[:,[len(trainX),-1]]

#process LP
I_matrix = np.ones_like(T_uu)
inv_matrix = np.linalg.inv(I_matrix-T_uu)
Y_labeled_matrix = [vectorized_result(y_l) for y_l in trainY]
Y_un_matrix = inv_matrix*T_ul*Y_labeled_matrix
