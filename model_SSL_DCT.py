...
def traindata_flip(trainX,trainY):
    ...
    pdb.set_trace()
    newX_h1 = tf.map_fn(lambda img: tf.image.flip_up_down(img), newX_h)
    newX_h = newX_h + newX_h1
    newX_h2 = tf.map_fn(lambda img: tf.image.flip_left_right(img), newX_h)
    #newX_h1 = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input)
    newX_h = newX_h +newX_h2
    newX = newX_h + newX_n
    newY = np.ones(len(newX_h))+np.zeros(len(newX_n))
    return newX, newY
...
def forward_new(input, is_training=True, reuse=False, scope='model', flip=False):
    #if flip == True:
        #input = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input)
        #input = tf.map_fn(lambda img: tf.image.random_flip_up_down(img), input)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1, padding='SAME',
                            weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                            biases_initializer=tf.constant_initializer(0.0)):
            net = slim.conv2d(inputs=input, num_outputs=8, kernel_size=[3, 3], scope='conv1_1')
            net = slim.conv2d(net, 8, [3, 3], scope='conv1_2')
            net = slim.max_pool2d(net, [2, 2], stride=2, padding='SAME', scope='pool1')
            #crosstask part
            ...
            net1 = slim.max_pool2d(net1, [2, 2], stride=2, padding='SAME', scope='pool2_1')
            net1 = slim.flatten(net1)
            net = slim.conv2d(net, 16, [3, 3], scope='conv2_1')
            net = slim.conv2d(net, 16, [3, 3], scope='conv2_2')
            net = slim.max_pool2d(net, [2, 2], stride=2, padding='SAME', scope='pool2')
            net = slim.flatten(net)
            w_init = tf.contrib.layers.xavier_initializer(uniform=False)
            net = slim.fully_connected(net, 250, activation_fn=tf.nn.relu, scope='fc1')
