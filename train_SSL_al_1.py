...
D_kl,C_loss = get_Dkl(predict1,P)
#C_loss = get_Closs(predict1,P,D_kl,margin)
...
v_num = 4 # un_loss(3) or v_loss(4)
un_loss_based = 0 #un_loss or v_loss
t_num = 4 #iteration round
al_ratio = 0.01 #ratio of data for al
ckpt   = True
...
with tf.Session(config=config) as sess:
    ...
    for t in xrange(t_num):
        ...
        for i in range(len(vi_un)):
            ...
            #active learning, assign labels to low confident samples
            if vi_un[i] == -1:
                unlabel_al.append(unlabel_whole[i])
                w_un_al.append(wi[i])
        #al, label and weight assignment
        if not r_index ==14:
            ...
            print("%d unlabeled data for al are used" % (len(unlabel_al_new)))
        else:
            newdata_fortrain = train_original + unlabel_fortrain
        ...
        if not r_index ==14:
            #w_new = np.ones(len_trainingX).tolist() + w_un_fortrain + w_al_new.tolist()
            #w_original
            w_new = np.ones(len_trainingX).tolist() + w_un_fortrain + w_un_al
        else:
            w_new = np.ones(len_trainingX).tolist() + w_un_fortrain
        #return(newdataX,newdataY,w_new)
        print("%d unlabeled data totally are used" % (len(newdataX)))
